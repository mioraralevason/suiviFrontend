...
$content = Get-Content -Path "D:\suivilbcft\src\components\Management\ScoringPage.tsx" -Raw
...
$pattern = [regex]::Escape('  const handleSaveRule = async (booleanData?: any) => {') + '.*?' + [regex]::Escape('};')
$updatedContent = [regex]::Replace($content, $pattern, $newFunction, [System.Text.RegularExpressions.RegexOptions]::Singleline)

# Write the updated content back to the file
Set-Content -Path "D:\suivilbcft\src\components\Management\ScoringPage.tsx" -Value $updatedContent -Encoding UTF8
