// src/components/scoring/FormDateRange.tsx
...
type Props = {
  rule?: {
    value?: string; // "2025-01-01-2025-12-31" ou "2025-06-15"
...
export const FormDateRange: React.FC<Props> = ({
  rule,
  index = -1,
  onDelete,
  isOpen,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (startDate && endDate) {
      condition = `reponse::DATE BETWEEN '${startDate}' AND '${endDate}'`;
    } else if (startDate) {
      condition = `reponse::DATE = '${startDate}'`;
    } else {
      condition = `reponse::DATE = '${endDate}'`;
    }
      ...
      <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-screen overflow-y-auto p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-lg font-semibold text-gray-900">
            {isEdit ? 'Modifier la règle - Intervalle de dates' : 'Ajouter une règle - Intervalle de dates'}
          </h3>
          <button
            onClick={onClose}
        ...
        <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
          <p className="text-sm text-blue-800">
            <strong>Note :</strong> Saisissez une plage (ex: 01/01/2025 à 31/12/2025) ou une date unique.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} noValidate className="space-y-6">
          ...
          <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
            ...
            <p className="text-xs text-blue-700 mt-2">
              Laissez un champ vide pour une date unique.
            </p>
            ...
            {(startDate || endDate) && (
              <div className="mt-3 p-2 bg-blue-100 rounded font-mono text-xs text-blue-900 break-all">
                {startDate && endDate
                  ? `reponse::DATE BETWEEN '${startDate}' AND '${endDate}'`
                  : `reponse::DATE = '${startDate || endDate}'`}
              </div>
            )}
          </div>
