// src/components/scoring/FormTextLong.tsx
...
type Props = {
  rule?: {
    value?: string; // "rapport" ou "=exact" ou "~^start" ou "!~*ignore"
...
const OPERATORS = [
  { value: 'ILIKE', label: 'Contient (insensible à la casse)' },
  { value: '=', label: 'Égal à (exact)' },
  { value: '~', label: 'Correspond à (regex)' },
  { value: '!~*', label: 'Ne contient pas (insensible)' },
] as const;

export const FormTextLong: React.FC<Props> = ({
  rule,
  index = -1,
  onDelete,
  isOpen,
  ...
  // États locaux
  const [operator, setOperator] = useState<'ILIKE' | '=' | '~' | '!~*'>('ILIKE');
  ...
  useEffect(() => {
    if (rule?.value) {
      // Cas opérateur + valeur → "=exact"
      const opMatch = rule.value.match(/^(ILIKE|~|!~\*|=?)(.+)$/i);
      if (opMatch) {
        ...
        // Cas par défaut : ILIKE
        setOperator('ILIKE');
        setTextValue(rule.value.trim());
      }
    }
    if (rule?.points) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const escapedValue = textValue.replace(/'/g, "''"); // Échappement SQL

    if (operator === 'ILIKE') {
      condition = `reponse ILIKE '%${escapedValue}%'`;
    } else if (operator === '=') {
      condition = `reponse = '${escapedValue}'`;
    } else if (operator === '~') {
      condition = `reponse ~ '${escapedValue}'`;
    } else {
      condition = `reponse !~* '${escapedValue}'`;
      ...
      <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-screen overflow-y-auto p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-lg font-semibold text-gray-900">
            {isEdit ? 'Modifier la règle - Texte long' : 'Ajouter une règle - Texte long'}
          </h3>
          <button
            onClick={onClose}
        ...
        <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
          <p className="text-sm text-blue-800">
            <strong>Note :</strong> Recherchez un mot, une phrase exacte ou une expression régulière.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} noValidate className="space-y-6">
          ...
          <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
            ...
            <div className="space-y-3">
              <div className="flex gap-3 items-start">
                ...
                <div className="flex-1">
                  <textarea
                    ...
                    placeholder={
                      operator === 'ILIKE' ? 'ex: rapport annuel' :
                      operator === '=' ? 'ex: Non applicable' :
                      operator === '~' ? 'ex: ^[A-Z]{2}\\d+' :
                      'ex: confidential'
                    }
                    rows={3}
              ...
              {/* Aperçu SQL */}
              {textValue && (
                <div className="mt-3 p-2 bg-blue-100 rounded font-mono text-xs text-blue-900 break-all">
                  {operator === 'ILIKE' && `reponse ILIKE '%${textValue}%'`}
                  {operator === '=' && `reponse = '${textValue}'`}
                  {operator === '~' && `reponse ~ '${textValue}'`}
                  {operator === '!~*' && `reponse !~* '${textValue}'`}
                </div>
              )}
