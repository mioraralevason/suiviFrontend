// src/components/scoring/FormTextShort.tsx
...
type Props = {
  rule?: {
    value?: string; // "oui" ou "=exact" ou "~^A" ou "!~*confidential"
...
const OPERATORS = [
  { value: 'ILIKE', label: 'Contient (insensible)' },
  { value: '=', label: 'Égal à (exact)' },
  { value: '~', label: 'Correspond à (regex)' },
  { value: '!~*', label: 'Ne contient pas' },
] as const;

export const FormTextShort: React.FC<Props> = ({
  rule,
  index = -1,
  onDelete,
  isOpen,
      ...
      <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-screen overflow-y-auto p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-lg font-semibold text-gray-900">
            {isEdit ? 'Modifier la règle - Texte court' : 'Ajouter une règle - Texte court'}
          </h3>
          <button
            onClick={onClose}
        ...
        <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
          <p className="text-sm text-blue-800">
            <strong>Note :</strong> Recherchez un mot, une valeur exacte ou une expression.
          </p>
        </div>
